...
if [[ "$DEPLOY_MODE" == --apply && "$HEALTH_CHECK" == 1 &&
  ...
  die 'custom SSH target requires --health-url or --skip-health-check on apply'
fi
...
if [[ "$DEPLOY_MODE" == --apply && "$HEALTH_CHECK" == 1 ]]; then
  ...
  printf 'Public demo health check: skipped\n'
fi
