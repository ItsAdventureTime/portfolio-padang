...
export default function LoginPage() {
  ...
  if (isDemo) {
    return <main className="session-gate"><section className="panel session-gate-panel"><div className="eyebrow">Demo mode</div><h1 className="page-title">No login required</h1><p className="page-copy">The demonstration environment uses synthetic data and a guarded role selector. Production authentication is intentionally not part of this build.</p><Link className="primary-button" href="/" style={{ marginTop: 22 }}>Open demo dashboard</Link></section></main>;
  }

  return <main className="session-gate"><section className="panel session-gate-panel" aria-labelledby="login-title"><div className="eyebrow">Padang ERP Lite</div><h1 id="login-title" className="page-title">Sign in securely.</h1><p className="page-copy">Enter your approved work email. We&apos;ll send a one-time code; no password is stored.</p>{error && <div className="login-error" role="alert">{error}</div>}<div className="sr-only" role="status" aria-live="polite">{status}</div>{submitted ? <form onSubmit={verifyCode} style={{ marginTop: 22, display: 'grid', gap: 12 }} aria-describedby="code-help"><div className="status-pill status-green">If the account is approved, a code is on its way.</div><p id="code-help" className="login-help">{expired ? 'This code has expired. Request a new code to continue.' : `The code expires in about ${expiryMinutes} minute${expiryMinutes === 1 ? '' : 's'}.`}</p><label htmlFor="code" style={{ fontSize: 12, fontWeight: 700 }}>One-time code</label><input ref={codeRef} id="code" name="code" required inputMode="numeric" pattern="[0-9]{6}" maxLength={6} autoComplete="one-time-code" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000" aria-invalid={Boolean(error)} style={{ border: '1px solid var(--line)', borderRadius: 8, padding: '11px 12px', letterSpacing: '0.25em' }} /><button className="primary-button" type="submit" disabled={busy || expired}>{busy ? 'Verifying…' : expired ? 'Code expired' : 'Verify and sign in'}</button><button className="secondary-button" type="button" onClick={() => void submitRequest()} disabled={busy || cooldownSeconds > 0}>{cooldownSeconds > 0 ? `Resend code in ${cooldownSeconds}s` : 'Resend code'}</button><button type="button" onClick={restart} className="text-button text-button-muted">Use a different email</button></form> : <form onSubmit={requestCode} style={{ marginTop: 22, display: 'grid', gap: 12 }}><label htmlFor="email" style={{ fontSize: 12, fontWeight: 700 }}>Work email</label><input ref={emailRef} id="email" name="email" required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@company.com" aria-invalid={Boolean(error)} style={{ border: '1px solid var(--line)', borderRadius: 8, padding: '11px 12px' }} /><button className="primary-button" type="submit" disabled={busy || cooldownSeconds > 0}>{busy ? 'Sending…' : cooldownSeconds > 0 ? `Try again in ${cooldownSeconds}s` : 'Send one-time code'}</button></form>}</section></main>;
}
