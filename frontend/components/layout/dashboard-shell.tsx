...
const roles = [
  ['administrator', 'Administrator'], ['general_manager', 'General Manager'],
  ['disbursing_check_signing_officer', 'DCS Officer'], ['project_manager', 'Project Manager'],
  ['procurement_officer', 'Procurement Officer'], ['fabrication_supervisor', 'Fabrication Supervisor'],
  ['finance_staff', 'Finance Staff'], ['billing_clerk', 'Billing Clerk'],
  ['inventory_clerk', 'Inventory Clerk'], ['viewer', 'Viewer'],
] as const;
const navigation = [
...
export function DashboardShell() {
  ...
  const roleName = roles.find(([value]) => value === role)?.[1] ?? 'General Manager';
  return <div className="shell-grid">
    ...
    <div className="main">
      <div className="demo-banner">Demo mode · Synthetic data resets every 30 minutes · No login required</div>
      <header className="topbar"><div className="top-actions"><button className="mobile-menu" onClick={() => setDrawerOpen(true)} aria-label="Open navigation"><Menu size={18} /></button><div className="breadcrumb">Workspace <ChevronRight size={13} style={{ display: 'inline', verticalAlign: 'middle' }} /> <strong>Dashboard</strong></div></div><div className="top-actions"><Bell aria-label="Notifications" size={17} color="#64748b" /><label className="role-label" htmlFor="role">Demo role</label><select className="role-select" id="role" value={role} onChange={(event) => setRole(event.target.value)}>{roles.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div></header>
...
export function WorkspaceFrame({ children }: { children: ReactNode }) {
  ...
  return <div className="shell-grid">
    ...
    <div className="main">
      <div className="demo-banner">Demo mode · Synthetic data resets every 30 minutes · No login required</div>
      <header className="topbar"><div className="top-actions"><button className="mobile-menu" onClick={() => setDrawerOpen(true)} aria-label="Open navigation"><Menu size={18} /></button><div className="breadcrumb">Workspace <ChevronRight size={13} style={{ display: 'inline', verticalAlign: 'middle' }} /> <strong>Operations</strong></div></div><div className="top-actions"><Bell aria-label="Notifications" size={17} color="#64748b" /><label className="role-label" htmlFor="module-role">Demo role</label><select className="role-select" id="module-role" value={role} onChange={(event) => setRole(event.target.value)}>{roles.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div></header>
      {children}
    </div>
    <div className={`mobile-drawer ${drawerOpen ? 'open' : ''}`} role="dialog" aria-modal="true" aria-label="Mobile navigation" onClick={() => setDrawerOpen(false)}><div className="mobile-drawer-panel" onClick={(event) => event.stopPropagation()}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><Brand /><button className="mobile-menu" onClick={() => setDrawerOpen(false)} aria-label="Close navigation"><X size={18} /></button></div><Navigation onNavigate={() => setDrawerOpen(false)} /></div></div>
  </div>;
