...
export function ModuleWorkspace({ module }: { module: ModuleKey }) {
  ...
  return <main id="workspace-main" className="content module-content">
    ...
    <section className="page-heading motion-enter"><div><div className="eyebrow">{config.eyebrow}</div><h1 className="page-title">{config.title}</h1><p className="page-copy">{config.description}</p></div><span className="read-only-action" title="Record creation is not connected in this build">Read-only register</span></section>
    <div className={`data-status motion-enter ${requestState === 'error' ? 'data-status-error' : ''}`} role={requestState === 'error' ? 'alert' : 'status'} aria-live="polite">
    ...
    {requestState === 'unavailable' && !hasAccess ? <section className="panel module-access" aria-live="polite"><h2 className="panel-title">Module hidden for this role</h2><p>{demoPermissions(role).summary}</p><Link className="secondary-button" href="/">Return to dashboard</Link></section> : <>
      {isDemo && <section className="stats-grid motion-stagger" aria-label={`${config.title} demo summary`}>{config.cards.map(([value, label, meta]) => <article className="stat-card motion-item" key={label}><div className="stat-label">{label}</div><div className="stat-value">{value}</div><div className="stat-meta stat-meta-muted">{meta}</div></article>)}</section>}
          <section className="panel panel-spaced motion-enter" aria-busy={requestState === 'loading'}><div className="panel-header"><div><h2 className="panel-title">{config.title} register</h2><p className="panel-subtitle">{statusCopy}</p></div><label className="search-field" htmlFor={`${module}-search`}><Search size={15} aria-hidden="true" /><span className="sr-only">Search {config.title}</span><input id={`${module}-search`} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search records" /></label></div><div className="table-wrap"><table className="data-table"><caption className="sr-only">{config.title} register</caption><thead><tr>{config.columns.map(([, label]) => <th key={label} scope="col">{label}</th>)}<th scope="col"><span className="sr-only">Actions</span></th></tr></thead><tbody>{requestState === 'loading' ? <SkeletonRows columnCount={config.columns.length} /> : filtered.map((row, index) => <tr key={String(row[config.columns[0][0]] ?? index)}>{config.columns.map(([key, label]) => <td data-label={label} key={key} className={key.includes('amount') || key.includes('price') || key.includes('cost') ? 'font-mono' : ''}>{cellValue(key, row[key])}</td>)}<td data-label="Actions"><span className="read-only-control" title="Record details are not available in this build"><ArrowUpRight aria-hidden="true" size={15} /><span className="sr-only">Details are read-only</span></span></td></tr>)}</tbody></table></div>{liveEmpty && <div className="empty-state"><strong>No live records yet.</strong><span>This register is connected, but the API returned no records.</span></div>}{searchEmpty && <div className="empty-state"><strong>No matches for “{query}”.</strong><span>Try a project code, status, or reference number.</span></div>}{requestState === 'preview' && <div className="empty-state"><strong>Preview rows are illustrative only.</strong><span>They are not saved and cannot be opened in this build.</span></div>}{requestState === 'unavailable' && hasAccess && <div className="empty-state"><strong>Register connection pending.</strong><span>No production rows are shown until this register is connected.</span></div>}</section>
    </>}
  </main>;
}
